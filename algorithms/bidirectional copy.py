...
def search(agent:'Agent', all:bool=False) -> dict[list[str], 'Cell', int] | int:
    ...
    while open_list_start and open_list_goal:
        ...
        if current_start == current_goal:
            ...
            parent_start = parent_goal = None
            # current_parent = current_start.parent
            # old_parent = old_parents[current_start] if current_start in old_parents else None
            if current_start.parent in closed_set_start:
                parent_start = current_start.parent
                parent_goal = old_parents[current_start]
            else:
                parent_start = old_parents[current_start]
                parent_goal = current_start.parent
            path_start = agent.trace_path(parent_start)
            connect(agent, parent_goal, parent_start)
            path_goal = agent.trace_path(parent_start, backward=False)
        ...
        for neighbor in agent.grid.get_neighbors(current_start, agent.can_jump):
            ...
            if neighbor not in open_list_start:
                ...
                if neighbor.parent:  # If the neighbor has a parent, a path is found
                    ...
                    # collapsed = True
                    old_parents[neighbor] = neighbor.parent
                    # start.parent = goal.parent = None
                    # Extend the path
        ...
        for neighbor in agent.grid.get_neighbors(current_goal, agent.can_jump):
            ...
            if neighbor not in open_list_goal:
                count += 1
                if neighbor == current_start:
                    collapsed = True
                if neighbor.parent and not collapsed:
